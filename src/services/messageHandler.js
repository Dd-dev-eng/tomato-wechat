...
class MessageHandler {
  async handle(msg) {
    ...
    const openid = FromUserName;       // 用户的 openid
    const accountId = ToUserName;      // 公众号的 ID
    ...
    try {
      ...
      if (MsgType === 'event') {
        return textReply(openid, accountId, '🍅 欢迎使用主动番茄！\n\n发送「开始」开启一个番茄钟');
      }
      return textReply(openid, accountId, '发送「开始」开始专注吧');
    } catch (e) {
  ...
  async handleText(openid, accountId, content) {
    ...
    switch (session.step) {
      ...
      default: return textReply(openid, accountId, '发送「开始」开启一个番茄钟 🍅');
    }
  }
